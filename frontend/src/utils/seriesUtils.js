...
export const getSeriesBaseName = (name) => {
    ...
    return name
        ...
        .replace(/\s*(\d{1,2}\s*[xX]\s*\d{1,2}|[xX]\d{1,2}).*/i, '')
        .replace(/\s*(?:temporada|season|temp|temp\.|epis[oó]dio|episode|capitulo|capítulo|ep|ep\.|e)\s*\d+.*/i, '')
        
        // 2. Remove anos entre parênteses ou colchetes (se vierem depois do nome)
        .replace(/\s*[\(\[]\s*\d{4}\s*[\)\]].*/g, '')
        
        // 3. REMOÇÃO AGRESSIVA DE TAGS TÉCNICAS E QUALIDADE
        ...
        .replace(/\s*\b(netflix|amazon|disney|apple|hbo|globoplay|prime|video)\b/gi, '')
